...
5.  **Gated Conversion:** Keep high-value data (line items, specific buyers) behind a login/register wall to drive user acquisition. Redirect auth actions to central `loxtr.com/login` and `loxtr.com/register` with a `redirectTo` parameter pointing back to the current tool.
