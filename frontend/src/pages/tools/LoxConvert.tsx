                    ...
                    <motion.div key="landing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="max-w-7xl mx-auto px-6 pt-24 md:pt-32">
                        <div className="text-center mb-20">
                            ...
                            <div className="max-w-4xl mx-auto mb-24 px-4">
                                <div className="bg-white rounded-[5rem] p-8 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] border-t-[16px] border-yellow group relative overflow-hidden transition-all hover:shadow-yellow/10">
                                    <div className={`relative border-2 border-dashed rounded-[4rem] p-20 md:p-36 text-center transition-all duration-500
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col items-center">
                                            <div className={`w-36 h-36 rounded-[3rem] flex items-center justify-center mb-10 transition-all shadow-2xl
                                                ${loading ? 'bg-navy text-yellow ring-[12px] ring-yellow/10 animate-pulse' : 'bg-white text-navy border border-slate-100 group-hover:scale-110'}`}
                                            >
                                                {loading ? <Loader2 size={56} className="animate-spin" /> : <Layers size={56} />}
                                            </div>
                                            <h3 className="text-4xl md:text-5xl font-black text-navy mb-4 italic tracking-tighter uppercase leading-none">
                                                {loading ? 'Analyzing...' : 'Shipment Dossier'}
                                            </h3>
                                            <p className="text-slate-400 text-[12px] font-black uppercase tracking-[0.6em] opacity-60">Vision-LLM Global Trade Radar</p>
                                        </div>
                                    </div>
                    ...
                    <div className="fixed inset-0 z-[400] flex items-center justify-center p-8 bg-black/60 backdrop-blur-xl">
                        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[5rem] p-16 max-w-xl w-full shadow-2xl text-center">
                            ...
                            <div className="flex flex-col gap-4">
                                <button onClick={() => navigate(`/register?redirectTo=${encodeURIComponent(window.location.href)}`)} className="w-full bg-navy text-white py-6 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-yellow hover:text-navy transition-all">Create Free Account</button>
                                <button onClick={() => navigate(`/login?redirectTo=${encodeURIComponent(window.location.href)}`)} className="w-full bg-slate-100 text-navy py-6 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all">Already a Member? Login</button>
                                <button onClick={() => setShowAuthGate(false)} className="w-full py-4 rounded-2xl text-[10px] font-black uppercase text-slate-300 hover:text-navy transition-all tracking-[0.2em]">Not Now, Keep Analyzing</button>
                            </div>
                        </motion.div>
                    </div>
                )}
